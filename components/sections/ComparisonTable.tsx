...
const rows = [
  { label: 'Актуальные материалы с сертификатом ФИПИ', tutor: null, course: null, max: true },
  { label: 'Контроль посещения занятий и прохождения всей программы', tutor: "нет стандартизированной программы ученик пройдет, сколько успеет", course: "контролирует куратор/nнакапливается много пропущенных занятий", max: "Ученики посещают 90% занятий вживую" },
  { label: 'Мониторинг активности учеников на занятии', tutor: "Не автоматизированно", course: null, max: "Платформа Мкласс фиксирует активность и помогает не отвлекаться" },
  ...
  { label: 'Преподаватель каждый год сдает ЕГЭ и подтверждает свой уровень', tutor: null, course: 'иногда', max: true },
  { label: 'Личный кабинет для родителей', tutor: null, course: null, max: true },
  { label: 'Психологическая поддержка', tutor: null, course: "иногда", max: 'семинары и личные консультации с ментором' },
