...
const pillars = [
  {
    ...
    body: 'Только 3% кандидатов получают право проводить занятия. Экзамен по предмету, пробный урок, работа над методикой.',
    badge: '3% кандидатов проходят отбор',
  },
  {
    icon: MessageCircle,
    title: 'Каждый ученик работает на уроке',
    body: 'Не получится «отсидеться». Преподаватель распределяет задания равномерно — каждый думает, отвечает и разбирает ошибки.',
    badge: 'Баланс в группе',
  },
  {
    icon: Users,
    title: 'Преподаватель знает каждого ученика',
    body: 'Небольшая группа — это про результат. Видно, где пробел, где притормозить, а где двигаться быстрее.',
    badge: 'Небольшие группы',
  },
]

export default function EgeMechanism() {
  return (
